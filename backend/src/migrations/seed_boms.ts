...
async function seedBOM() {
  try {
    ...
    for (const comp of components) {
      const [existing]: any = await pool.query('SELECT id FROM inventory_stok WHERE kode_barang = ?', [comp.kode_barang]);
      if (existing.length === 0) {
        await pool.query(
          'INSERT INTO inventory_stok (kode_barang, nama_barang, kategori, tipe_item, jumlah_stok, satuan) VALUES (?, ?, ?, ?, ?, ?)',
          [comp.kode_barang, comp.nama_barang, comp.kategori, comp.tipe_item, comp.jumlah_stok, comp.satuan]
        );
      }
    }
    ...
    for (const bike of bicycles) {
      // Create Header
      const [existingBom]: any = await pool.query('SELECT id_bom FROM manufaktur_bom_header WHERE id_bom = ?', [bike.id_bom]);
      if (existingBom.length === 0) {
        await pool.query(
          'INSERT INTO manufaktur_bom_header (id_bom, kode_item_parent, nama_resep) VALUES (?, ?, ?)',
          [bike.id_bom, bike.kode, bike.nama]
        );
        ...
        for (const detail of bomDetails) {
          await pool.query(
            ...
            [bike.id_bom, detail.kode_item_komponen, detail.qty_kebutuhan]
          );
        }
