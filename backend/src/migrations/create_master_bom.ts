...
async function migrate() {
  try {
    ...
    const [existingFG]: any = await pool.query(`SELECT id FROM inventory_stok WHERE kode_barang = 'FG-001'`);
    let id_sepeda;
    ...
    const wipIds: Record<string, number> = {};
    for (const wip of wips) {
      const [existing]: any = await pool.query(`SELECT id FROM inventory_stok WHERE kode_barang = ?`, [wip.kode]);
    ...
    const getKompId = async (kode: string) => {
      const [rows]: any = await pool.query(`SELECT id FROM inventory_stok WHERE kode_barang = ?`, [kode]);
      return rows[0]?.id;
    };
