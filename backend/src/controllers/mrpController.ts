...
class RecoAggregator {
  ...
  addPO(kode: string, nama: string, qty: number) {
    if (this.poMap.has(kode)) {
      this.poMap.get(kode)!.qty += qty;
    } else {
      this.poMap.set(kode, { kode_barang: kode, nama_barang: nama, qty });
    }
  }
}
...
async function explodeBOM(
  ...
  const [stokData]: any = await connection.query(
    'SELECT nama_barang, tipe_item, jumlah_stok, stok_committed, harga_standar FROM inventory_stok WHERE kode_barang = ?',
    [kodeItem]
  );
  ...
  if (stok.tipe_item === 'RM') {
    ...
    aggregator.addPO(kodeItem, stok.nama_barang, qtyTarget);
  } 
  ...
  try {
    ...
    for (const wo of wo_list) {
      ...
      await connection.query(
        ...
        [nomor_wo, wo.kode_sepeda, wo.qty, 'Menunggu']
      );
    }

    // 2. Eksekusi PO Recommendations
    for (const po of po_list) {
      ...
      await connection.query(
        ...
        [nomor_pr, po.nama_barang, po.qty, 'pcs', 'TBD - Menunggu Tim Purchasing', 'Menunggu Persetujuan']
      );
    }
...
export const getMasterBOM = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [headers]: any = await pool.query(`
      SELECT b.id_bom, b.kode_item_parent, b.nama_resep, i.nama_barang 
    ...
    // Mapping ke struktur nested (Tree)
    const bomTree = headers.map((header: any) => {
