...
// ============================================================
export async function generatePONumber(connection) {
    const date = new Date();
    const yy = date.getFullYear().toString().slice(-2);
    const mm = (date.getMonth() + 1).toString().padStart(2, '0');
    const prefix = `PO-MTK-${yy}${mm}-`;
    const [rows] = await connection.query(`SELECT nomor_po FROM pengadaan_po_header WHERE nomor_po LIKE ? ORDER BY id DESC LIMIT 1`, [`${prefix}%`]);
    let nextNum = 1;
    if (rows.length > 0) {
...
// ============================================================
export const createDirectPO = async (req, res) => {
    ...
    try {
        ...
        // Insert Header
        const [headerResult] = await connection.query(`INSERT INTO pengadaan_po_header (nomor_po, id_vendor, status, total_nilai, catatan) VALUES (?, ?, ?, ?, ?)`, [nomor_po, id_vendor, 'DRAFT', total_nilai, catatan]);
        const poId = headerResult.insertId;
        // Insert Details
        for (const item of validItems) {
            await connection.query(`INSERT INTO pengadaan_po_detail (id_po_header, id_inventory_material, qty, harga_satuan) VALUES (?, ?, ?, ?)`, [poId, item.id_inventory_material, item.qty, item.harga]);
        }
...
export const updatePOStatus = async (req, res) => {
    ...
    try {
        ...
        await connection.beginTransaction();
        const [poRows] = await connection.query('SELECT * FROM pengadaan_po_header WHERE id = ? FOR UPDATE', [poId]);
        if (poRows.length === 0)
        ...
        if (status === 'ISSUED' && currentStatus === 'DRAFT') {
            if (role !== 'Pengadaan' && role !== 'Owner' && role !== 'General Manager')
                throw new Error('Akses ditolak: Hanya Pengadaan yang bisa mengajukan PO.');
            ...
            if (role !== 'General Manager' && role !== 'Owner')
                throw new Error('Akses ditolak: Hanya Executive yang bisa menyetujui/menolak PO.');
            ...
            if (role !== 'General Manager' && role !== 'Owner')
                throw new Error('Akses ditolak: Hanya Executive yang bisa menyetujui PO.');
            ...
            if (role !== 'Pengadaan' && role !== 'Owner' && role !== 'General Manager')
                throw new Error('Akses ditolak: Hanya Pengadaan yang bisa mengirim ke vendor.');
            await connection.query('UPDATE pengadaan_po_header SET status = "SENT_TO_VENDOR" WHERE id = ?', [poId]);
...
export const generatePO = async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        // 1. Dapatkan PR
        const [prs] = await connection.query('SELECT * FROM pengadaan_pr_header WHERE id = ?', [id_pr]);
        if (prs.length === 0)
            throw new Error('PR tidak ditemukan');
        const pr = prs[0];
        if (pr.status_pr === 'Selesai')
            throw new Error('Aksi Ditolak: PR ini sudah pernah di-generate menjadi PO sebelumnya!');
        if (pr.status_pr !== 'Diproses Vendor')
            throw new Error('Akses ditolak: PR belum disetujui untuk diproses (Status harus Diproses Vendor)');
        ...
        const [poInsert] = await connection.query('INSERT INTO pengadaan_po_header (nomor_po, id_vendor, status, catatan) VALUES (?, ?, ?, ?)', [nomor_po, pr.id_vendor, 'DRAFT', `Generated from PR ${pr.nomor_pr}`]);
...
export const bulkGeneratePO = async (req, res) => {
    // Ambil semua PR yang statusnya 'Diproses Vendor' dan generate PO
    const userId = req.user.id;
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const [prs] = await connection.query('SELECT * FROM pengadaan_pr_header WHERE status_pr = ?', ['Diproses Vendor']);
        if (prs.length === 0) {
        ...
        for (const pr of prs) {
            ...
            if (prDetails.length === 0)
                continue;
            const nomor_po = await generatePONumber(connection);
            const [poInsert] = await connection.query('INSERT INTO pengadaan_po_header (nomor_po, id_vendor, status, catatan) VALUES (?, ?, ?, ?)', [nomor_po, pr.id_vendor, 'DRAFT', `Bulk Generated from PR ${pr.nomor_pr}`]);
            const poId = poInsert.insertId;
            let total_nilai = 0;
...
// ============================================================
export const bulkReceivePO = async (req, res) => {
    const connection = await pool.getConnection();
    const user = req.user;
    const role = user?.divisi_role;
    try {
        ...
        const [pos] = await connection.query('SELECT id, nomor_po, id_pr FROM pengadaan_po_header WHERE status = "SENT_TO_VENDOR"');
        if (pos.length === 0) {
            await connection.rollback();
...
// ============================================================
export const bulkApprovePO = async (req, res) => {
    const connection = await pool.getConnection();
    const user = req.user;
    const role = user?.divisi_role;
    try {
        ...
        await connection.beginTransaction();
        const [pos] = await connection.query('SELECT id, nomor_po FROM pengadaan_po_header WHERE status = "ISSUED"');
        if (pos.length === 0) {
            await connection.rollback();
