...
async function explodeBOM(connection, kodeItem, qtyTarget, aggregator) {
    // 1. Cek tipe item dan stok di Master Inventory
    const [stokData] = await connection.query('SELECT nama_barang, tipe_item, jumlah_stok, stok_committed, harga_standar, id_vendor FROM inventory_stok WHERE kode_barang = ?', [kodeItem]);
    ...
    if (stok.tipe_item === 'RM') {
        ...
        if (deficit > 0) {
            ...
            // Cari resep BOM
            const [bomHeader] = await connection.query('SELECT id_bom FROM manufaktur_bom_header WHERE kode_item_parent = ?', [kodeItem]);
            ...
            const [bomDetails] = await connection.query('SELECT kode_item_komponen, qty_kebutuhan FROM manufaktur_bom_detail WHERE id_bom = ?', [idBom]);
...
// ============================================================
export const testExplodeBOM = async (req, res) => {
...
// ============================================================
export const executeMRP = async (req, res) => {
    const connection = await pool.getConnection();
    try {
        ...
        for (const wo of wo_list) {
            ...
            const [invLookup] = await connection.query('SELECT id FROM inventory_stok WHERE kode_barang = ?', [wo.kode_sepeda]);
            if (invLookup.length === 0)
                throw new Error(`Item ${wo.kode_sepeda} tidak ditemukan di Master Gudang.`);
            await connection.query('INSERT INTO operasi_wo_header (nomor_wo, id_inventory_fg, jumlah_produksi, status) VALUES (?, ?, ?, ?)', [nomor_wo, invLookup[0].id, wo.qty, 'ON_PROGRESS']);
        }
        ...
        for (const [vendorId, items] of groupedPOs.entries()) {
            ...
            for (const po of items) {
                await connection.query('INSERT INTO pengadaan_pr_detail (id_pr_header, kode_barang, jumlah, satuan) VALUES (?, ?, ?, ?)', [prHeaderId, po.kode_barang, po.qty, 'pcs']);
            }
        }
...
// ============================================================
export const getMasterBOM = async (req, res) => {
    const connection = await pool.getConnection();
    try {
        // Ambil header
    ...
    `);
        ...
        for (const header of headers) {
            const children = details.filter((d) => d.id_bom === header.id_bom);
...
export const updateCosts = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('UPDATE inventory_stok SET biaya_rakit = ?, biaya_antar = ? WHERE kode_barang = ?', [rakit, antar, kode_barang]);
...
// ============================================================
export const getExplodableItems = async (req, res) => {
    try {
        const [rows] = await pool.query("SELECT kode_barang, nama_barang, tipe_item FROM inventory_stok WHERE tipe_item IN ('FG', 'SA')");
