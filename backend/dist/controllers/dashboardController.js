...
export const getDashboard = async (req, res) => {
    try {
        ...
        // === KPI 3: Job Selesai ===
        const [jobRows] = await pool.query("SELECT COUNT(*) as total FROM operasi_kanban WHERE status_kolom = 'selesai'");
        const jobData = jobRows;
        if (jobData.length > 0) {
            response.kpi.job_selesai = Number(jobData[0].total) || 0;
        }
        // === KPI 4: Stok Komponen ===
        const [stokRows] = await pool.query("SELECT SUM(jumlah_stok) as total FROM inventory_stok WHERE kategori = 'Komponen'");
        const stokData = stokRows;
        if (stokData.length > 0) {
            response.kpi.stok_komponen = Number(stokData[0].total) || 0;
        }
        // === Chart 1: Distribusi Status Kanban (Bar Chart) ===
        const [kanbanRows] = await pool.query('SELECT status_kolom, COUNT(*) as jumlah FROM operasi_kanban GROUP BY status_kolom');
        const kanbanData = kanbanRows;
        ...
        const [stokKatRows] = await pool.query('SELECT kategori, SUM(jumlah_stok) as total FROM inventory_stok GROUP BY kategori');
        const stokKatData = stokKatRows;
