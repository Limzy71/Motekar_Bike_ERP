...
export const getAllStok = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, kode_barang, nama_barang, kategori, jumlah_stok, stok_committed, (jumlah_stok - stok_committed) as stok_available, satuan, reorder_point, last_updated FROM inventory_stok ORDER BY jumlah_stok ASC, nama_barang ASC');
        res.json({
            success: true,
            data: rows
        });
