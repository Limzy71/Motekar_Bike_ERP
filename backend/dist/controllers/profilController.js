...
export const getMyProfile = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT id, username, nama_lengkap, email, divisi_role, foto_profil, created_at FROM users WHERE id = ?', [userId]);
        if (rows.length === 0) {
...
export const changePassword = async (req, res) => {
    try {
        ...
        if (req.user?.id) {
            await logAudit(req.user.id, 'Gagal memperbarui password.', req.ip, 'Failed');
        }
        res.status(500).json({ success: false, message: error.message });
    }
};
...
export const uploadAvatar = async (req, res) => {
    try {
        ...
        const [oldRows] = await pool.query('SELECT foto_profil FROM users WHERE id = ?', [userId]);
...
export const deleteAvatar = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT foto_profil FROM users WHERE id = ?', [userId]);
        if (rows.length === 0) {
            res.status(404).json({ success: false, message: 'User tidak ditemukan.' });
            return;
        }
...
export const requestEmailChange = async (req, res) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id FROM pengajuan_ganti_email WHERE id_user = ? AND status IN ("Pending_Approval", "Pending_Verification")', [user.id]);
        if (existing.length > 0) {
            await pool.query('UPDATE pengajuan_ganti_email SET email_baru = ?, status = ?, token_verifikasi = ?, updated_at = NOW() WHERE id = ?', [email_baru, status, token, existing[0].id]);
        }
        else {
            await pool.query('INSERT INTO pengajuan_ganti_email (id_user, email_baru, status, token_verifikasi) VALUES (?, ?, ?, ?)', [user.id, email_baru, status, token]);
        }
        let msg = '';
        ...
        if (req.user?.id) {
            await logAudit(req.user.id, 'Gagal mengajukan ganti email.', req.ip, 'Failed');
        }
        res.status(500).json({ success: false, message: error.message });
    }
};
...
export const verifyEmailChange = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT id, email_baru FROM pengajuan_ganti_email WHERE id_user = ? AND token_verifikasi = ? AND status = "Pending_Verification"', [user.id, token]);
        if (rows.length === 0) {
        ...
        if (req.user?.id) {
            await logAudit(req.user.id, 'Gagal memverifikasi ganti email.', req.ip, 'Failed');
        }
        res.status(500).json({ success: false, message: error.message });
    }
};
...
export const getAuditLogs = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT id, action, ip_address, status, DATE_FORMAT(created_at, "%d/%m/%Y %H:%i") as waktu FROM audit_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT 5', [userId]);
