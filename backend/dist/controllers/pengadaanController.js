...
// ============================================================
export async function generatePRNumber(connection) {
    const year = new Date().getFullYear();
    const prefix = `PR/MTK/${year}/`;
    const [rows] = await connection.query(`SELECT nomor_pr FROM pengadaan_pr_header WHERE nomor_pr LIKE ? ORDER BY id DESC LIMIT 1`, [`${prefix}%`]);
    let nextNum = 1;
    if (rows.length > 0) {
...
export const getAllPR = async (req, res) => {
    ...
    `);
        const result = headers.map((header) => ({
            ...header,
            items: details.filter((d) => d.id_pr_header === header.id)
        }));
        res.json({
            success: true,
            data: result
...
export const createPR = async (req, res) => {
    ...
    try {
        ...
        const [headerResult] = await connection.query('INSERT INTO pengadaan_pr_header (nomor_pr, id_vendor, status_pr) VALUES (?, ?, ?)', [generated_nomor_pr, id_vendor, 'Menunggu Persetujuan']);
        ...
        for (const item of pr_items) {
            ...
            const [itemRows] = await connection.query('SELECT id_vendor FROM inventory_stok WHERE kode_barang = ?', [item.kode_barang]);
            ...
            await connection.query('INSERT INTO pengadaan_pr_detail (id_pr_header, kode_barang, jumlah, satuan) VALUES (?, ?, ?, ?)', [prHeaderId, item.kode_barang, qty, item.satuan]);
        }
...
export const approvePR = async (req, res) => {
    ...
    try {
        ...
        await connection.beginTransaction();
        const [result] = await connection.query('UPDATE pengadaan_pr_header SET status_pr = ? WHERE id = ? AND status_pr = ?', ['Diproses Vendor', prId, 'Menunggu Persetujuan']);
        if (result.affectedRows > 0) {
            ...
            const [poInsert] = await connection.query('INSERT INTO pengadaan_po_header (nomor_po, id_vendor, status, catatan, id_pr) VALUES (?, ?, ?, ?, ?)', [nomor_po, pr.id_vendor, 'DRAFT', `Generated from PR ${pr.nomor_pr}`, prId]);
            const poId = poInsert.insertId;
            let total_nilai = 0;
            for (const detail of prDetails) {
                const [items] = await connection.query('SELECT id, harga_standar FROM inventory_stok WHERE kode_barang = ?', [detail.kode_barang]);
                if (items.length === 0)
                ...
                await connection.query('INSERT INTO pengadaan_po_detail (id_po_header, id_inventory_material, qty, harga_satuan) VALUES (?, ?, ?, ?)', [poId, id_inventory, qty, harga]);
            }
...
// ============================================================
export const bulkApprovePR = async (req, res) => {
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const [prs] = await connection.query('SELECT * FROM pengadaan_pr_header WHERE status_pr IN (?, ?)', ['Menunggu Persetujuan', 'Draft']);
        if (prs.length === 0) {
            await connection.rollback();
        ...
        for (const pr of prs) {
            ...
            if (prDetails.length === 0)
                continue;
            const nomor_po = await generatePONumber(connection);
            const [poInsert] = await connection.query('INSERT INTO pengadaan_po_header (nomor_po, id_vendor, status, catatan, id_pr) VALUES (?, ?, ?, ?, ?)', [nomor_po, pr.id_vendor, 'DRAFT', `Generated from PR ${pr.nomor_pr}`, pr.id]);
            const poId = poInsert.insertId;
            let total_nilai = 0;
            for (const detail of prDetails) {
                const [items] = await connection.query('SELECT id, harga_standar FROM inventory_stok WHERE kode_barang = ?', [detail.kode_barang]);
                if (items.length === 0)
                    throw new Error(`Item ${detail.kode_barang} tidak ditemukan`);
                const id_inventory = items[0].id;
                const harga = parseFloat(items[0].harga_standar || 0);
                const qty = detail.jumlah;
                total_nilai += (qty * harga);
...
export const deletePR = async (req, res) => {
    try {
        ...
        const [check] = await pool.query('SELECT status_pr FROM pengadaan_pr_header WHERE id = ?', [prId]);
        if (check.length === 0) {
    ...
    `, [prId]);
        ...
        const [result] = await pool.query('DELETE FROM pengadaan_pr_header WHERE id = ?', [prId]);
        if (result.affectedRows > 0) {
...
// ============================================================
export const bulkDeletePR = async (req, res) => {
    try {
...
// ============================================================
export const autoGeneratePR = async (req, res) => {
    const connection = await pool.getConnection();
    try {
        // 1. Get all deficit items that DON'T already have a pending PR
        const [deficitItems] = await connection.query(`
      SELECT 
    ...
    `);
        ...
        for (const vendorIdStr in groupedByVendor) {
            ...
            const [headerResult] = await connection.query('INSERT INTO pengadaan_pr_header (nomor_pr, id_vendor, status_pr) VALUES (?, ?, ?)', [nomor_pr, vendorId, 'Menunggu Persetujuan']);
            const prHeaderId = headerResult.insertId;
        ...
        if (orderedItemIds.length > 0) {
            await connection.query("UPDATE pengadaan_restock_requests SET status = 'Selesai' WHERE status = 'Pending' AND id_inventory_material IN (?)", [orderedItemIds]);
        }
        await connection.commit();
        res.status(201).json({
            success: true,
...
export const createRestockRequest = async (req, res) => {
    try {
        ...
        try {
            await connection.beginTransaction();
            for (const item of items) {
                await connection.query('INSERT INTO pengadaan_restock_requests (id_inventory_material, nomor_wo, jumlah_diminta, status) VALUES (?, ?, ?, ?)', [item.id_inventory_material, nomor_wo, item.jumlah_diminta, 'Pending']);
            }
            await connection.commit();
...
export const getPendingRequests = async (req, res) => {
    try {
        const [manualRequests] = await pool.query(`
      SELECT 
        r.id, 
    ...
    `);
        ...
        const [autoAlerts] = await pool.query(`
      SELECT 
...
export const completeRequest = async (req, res) => {
    try {
        ...
        const [result] = await pool.query("UPDATE pengadaan_restock_requests SET status = 'Selesai' WHERE id = ?", [numericId]);
        if (result.affectedRows > 0) {
