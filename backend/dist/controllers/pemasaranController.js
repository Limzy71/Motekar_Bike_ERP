...
export const deleteCampaign = async (req, res) => {
    try {
        ...
        const [leads] = await pool.query('SELECT COUNT(*) as count FROM pemasaran_leads WHERE id_campaign = ?', [id]);
        if (leads[0].count > 0) {
...
export const updateLeadStatus = async (req, res) => {
    try {
        ...
        const [currentLead] = await pool.query('SELECT status_pipeline FROM pemasaran_leads WHERE id_lead = ?', [leadId]);
        if (!currentLead.length) {
        ...
        const [result] = await pool.query('UPDATE pemasaran_leads SET status_pipeline = ? WHERE id_lead = ?', [status_pipeline, leadId]);
...
export const deleteLead = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('DELETE FROM pemasaran_leads WHERE id_lead = ?', [leadId]);
        if (result.affectedRows === 0) {
...
export const getAktivitasByLead = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT id_aktivitas, tanggal, jenis_interaksi, catatan_hasil FROM pemasaran_aktivitas WHERE id_lead = ? ORDER BY tanggal DESC', [leadId]);
        res.json({ success: true, data: rows });
    }
    catch (error) {
...
export const createAktivitas = async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO pemasaran_aktivitas (id_lead, jenis_interaksi, catatan_hasil) VALUES (?, ?, ?)', [parseInt(id_lead, 10), jenis_interaksi, catatan_hasil.trim()]);
...
export const getPemasaranKPI = async (req, res) => {
    try {
        // 1. Pipeline Value (Follow Up + Negosiasi)
        const [pipelineData] = await pool.query("SELECT COALESCE(SUM(estimasi_nilai_deal), 0) as total FROM pemasaran_leads WHERE status_pipeline IN ('Follow Up', 'Negosiasi')");
        const pipelineValue = parseFloat(pipelineData[0].total);
        // 2. Win Rate
        const [totalLeads] = await pool.query('SELECT COUNT(*) as total FROM pemasaran_leads');
        const [wonLeads] = await pool.query("SELECT COUNT(*) as total FROM pemasaran_leads WHERE status_pipeline = 'Won_Deal'");
        ...
        const [wonValue] = await pool.query("SELECT COALESCE(SUM(estimasi_nilai_deal), 0) as total FROM pemasaran_leads WHERE status_pipeline = 'Won_Deal'");
        const [totalBudget] = await pool.query('SELECT COALESCE(SUM(budget_alokasi), 0) as total FROM pemasaran_campaigns');
