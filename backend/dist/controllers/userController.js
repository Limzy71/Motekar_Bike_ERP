...
export const getAllUsers = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, username, nama_lengkap, email, divisi_role, status, created_at FROM users ORDER BY created_at DESC');
...
export const createUser = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO users (username, nama_lengkap, email, password, divisi_role, status) VALUES (?, ?, ?, ?, ?, ?)', [username, nama_lengkap, email || null, hashedPassword, divisi_role, 'Aktif']);
...
export const updateUser = async (req, res) => {
    try {
        ...
        // Ambil data target
        const [targetRows] = await pool.query('SELECT divisi_role FROM users WHERE id = ?', [targetId]);
        ...
        if (username) {
            await pool.query('UPDATE users SET username = ?, nama_lengkap = ?, divisi_role = ?, status = ?, email = ? WHERE id = ?', [username, nama_lengkap, divisi_role, status, email || null, targetId]);
        }
        else {
            await pool.query('UPDATE users SET nama_lengkap = ?, divisi_role = ?, status = ?, email = ? WHERE id = ?', [nama_lengkap, divisi_role, status, email || null, targetId]);
        }
...
export const resetPassword = async (req, res) => {
    try {
        ...
        // Ambil data target
        const [targetRows] = await pool.query('SELECT username, divisi_role FROM users WHERE id = ?', [targetId]);
        if (targetRows.length === 0) {
            res.status(404).json({ success: false, message: 'User tidak ditemukan.' });
            return;
        }
