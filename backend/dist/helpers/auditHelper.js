 ...
 */
export const logAudit = async (userId, action, ip = '127.0.0.1', status = 'Success') => {
    try {
        await pool.query('INSERT INTO audit_logs (user_id, action, ip_address, status) VALUES (?, ?, ?, ?)', [userId, action, ip, status]);
    }
    catch (error) {
