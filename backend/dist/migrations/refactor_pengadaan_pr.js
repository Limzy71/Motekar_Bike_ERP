...
async function up() {
    ...
    `);
      ...
      `, [pr1, pr2]);
            const [headerRows] = await connection.query('SELECT id, nomor_pr FROM pengadaan_pr_header');
            const idPr1 = headerRows.find((r) => r.nomor_pr === pr1).id;
            const idPr2 = headerRows.find((r) => r.nomor_pr === pr2).id;
            console.log('Fetching RM items from inventory_stok...');
            const [items] = await connection.query("SELECT kode_barang, satuan FROM inventory_stok WHERE tipe_item = 'RM' LIMIT 4");
            if (items.length >= 2) {
