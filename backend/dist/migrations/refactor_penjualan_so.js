...
async function up() {
    ...
    try {
        ...
        const [cols] = await connection.query("SHOW COLUMNS FROM inventory_stok LIKE 'harga_jual'");
        if (cols.length === 0) {
            await connection.query('ALTER TABLE inventory_stok ADD COLUMN harga_jual DECIMAL(15,2) NOT NULL DEFAULT 0');
    ...
    `);
        ...
        if (process.env.NO_SEED !== 'true') {
            ...
            const [campaigns] = await connection.query('SELECT id_campaign FROM pemasaran_campaigns LIMIT 1');
            let campaignId;
            if (campaigns.length === 0) {
                const [campResult] = await connection.query("INSERT INTO pemasaran_campaigns (nama_campaign, jenis, budget_alokasi, status) VALUES ('Pameran Inabicycle 2026', 'Pameran', 50000000, 'Aktif')");
            ...
            const [existingWon] = await connection.query("SELECT id_lead FROM pemasaran_leads WHERE status_pipeline = 'Won_Deal' LIMIT 1");
            let leadAId;
            if (existingWon.length === 0) {
                const [leadAResult] = await connection.query("INSERT INTO pemasaran_leads (nama_toko, kontak_person, no_telepon, id_campaign, estimasi_nilai_deal, status_pipeline) VALUES (?, ?, ?, ?, ?, 'Won_Deal')", ['PT Nusantara Jaya Sepeda', 'Budi Santoso', '0812-3456-7890', campaignId, 250000000]);
            ...
            const [existingNego] = await connection.query("SELECT id_lead FROM pemasaran_leads WHERE nama_toko = 'CV Angin Ribut Bicycles' LIMIT 1");
            if (existingNego.length === 0) {
                await connection.query("INSERT INTO pemasaran_leads (nama_toko, kontak_person, no_telepon, id_campaign, estimasi_nilai_deal, status_pipeline) VALUES (?, ?, ?, ?, ?, 'Negosiasi')", ['CV Angin Ribut Bicycles', 'Dina Pratama', '0856-9999-1234', campaignId, 75000000]);
            }
            ...
            const [fgItems] = await connection.query("SELECT kode_barang, harga_standar FROM inventory_stok WHERE tipe_item = 'FG'");
            for (const fg of fgItems) {
                ...
                await connection.query('UPDATE inventory_stok SET harga_jual = ? WHERE kode_barang = ?', [hargaJual, fg.kode_barang]);
            }
            ...
            const [so1Result] = await connection.query('INSERT INTO penjualan_so_header (no_so, id_customer, status) VALUES (?, ?, ?)', [noSO1, leadAId, 'Draft']);
            const so1Id = so1Result.insertId;
            // SO #2 — Confirmed, multi-item (validation target)
            const [so2Result] = await connection.query('INSERT INTO penjualan_so_header (no_so, id_customer, status) VALUES (?, ?, ?)', [noSO2, leadAId, 'Confirmed']);
            const so2Id = so2Result.insertId;
            // Fetch FG items for detail seeding
            const [fgForDetail] = await connection.query("SELECT kode_barang, harga_jual FROM inventory_stok WHERE tipe_item = 'FG' LIMIT 3");
            if (fgForDetail.length >= 1) {
                const fg1 = fgForDetail[0];
                const hpp1 = fg1.harga_jual * 0.6; // Simulate HPP ~60% of sell price
                // SO #1 detail: 1 item
                await connection.query('INSERT INTO penjualan_so_detail (id_so_header, kode_barang, qty, harga_jual_satuan, hpp_snapshot) VALUES (?, ?, ?, ?, ?)', [so1Id, fg1.kode_barang, 10, fg1.harga_jual, hpp1]);
                // SO #2 detail: multi-item
                await connection.query('INSERT INTO penjualan_so_detail (id_so_header, kode_barang, qty, harga_jual_satuan, hpp_snapshot) VALUES (?, ?, ?, ?, ?)', [so2Id, fg1.kode_barang, 10, fg1.harga_jual, hpp1]);
                if (fgForDetail.length >= 2) {
                    const fg2 = fgForDetail[1];
                    const hpp2 = fg2.harga_jual * 0.6;
                    await connection.query('INSERT INTO penjualan_so_detail (id_so_header, kode_barang, qty, harga_jual_satuan, hpp_snapshot) VALUES (?, ?, ?, ?, ?)', [so2Id, fg2.kode_barang, 5, fg2.harga_jual, hpp2]);
                }
            }
