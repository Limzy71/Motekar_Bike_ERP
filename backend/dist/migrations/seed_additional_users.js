...
async function seedAdditionalUsers() {
    ...
    try {
        ...
        for (const user of newUsers) {
            const [existingUser] = await connection.query('SELECT id FROM users WHERE username = ?', [user.username]);
            if (existingUser.length === 0) {
                await connection.query(`INSERT INTO users (username, password, nama_lengkap, email, divisi_role) 
           VALUES (?, ?, ?, ?, ?)`, [user.username, hashedPassword, user.nama_lengkap, user.email, user.divisi_role]);
