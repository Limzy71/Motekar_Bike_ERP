...
async function run() {
    ...
    try {
        ...
        for (const v of vendors) {
            await connection.query('INSERT INTO master_vendor (kode_vendor, nama_vendor, kategori, status_vendor) VALUES (?, ?, ?, ?)', [v.kode_vendor, v.nama_vendor, v.kategori, 'AKTIF']);
        }
        console.log('Vendors seeded.');
        // Seed Inventory
        for (const i of inventories) {
            await connection.query('INSERT INTO inventory_stok (kode_barang, nama_barang, kategori, satuan, tipe_item, harga_standar, harga_jual, jumlah_stok) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [i.kode, i.nama, i.kategori, i.satuan, i.tipe, i.harga, i.jual || 0, 100] // Beri stok awal 100 agar mudah testing WO
            );
        }
        console.log('Inventory seeded.');
        // Seed BOM Header
        for (const bh of bomHeaders) {
            await connection.query('INSERT INTO manufaktur_bom_header (id_bom, nama_resep, kode_item_parent) VALUES (?, ?, ?)', [bh.id_bom, bh.nama_bom, bh.kode_item_parent]);
        }
        console.log('BOM Headers seeded.');
        // Seed BOM Details
        for (const bd of bomDetails) {
            await connection.query('INSERT INTO manufaktur_bom_detail (id_bom, kode_item_komponen, qty_kebutuhan) VALUES (?, ?, ?)', [bd.id_bom, bd.kode_item_komponen, bd.qty]);
        }
