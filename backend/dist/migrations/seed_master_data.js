...
async function seedMasterData() {
    ...
    try {
        ...
        const [existingUser] = await connection.query('SELECT id FROM users WHERE username = ?', ['agus']);
        if (existingUser.length === 0) {
            const hashedPassword = await bcrypt.hash('password123', 10);
            await connection.query(`INSERT INTO users (username, password, nama_lengkap, email, divisi_role) 
         VALUES (?, ?, ?, ?, ?)`, ['agus', hashedPassword, 'Ir. Agus Hexagraha', 'agus@motekar.com', 'Owner']);
        ...
        const [existingVendor] = await connection.query("SELECT id FROM master_vendor WHERE nama_vendor = 'PT Shimano Indonesia'");
        if (existingVendor.length === 0) {
            await connection.query(`INSERT INTO master_vendor (nama_vendor, kontak, alamat) VALUES 
        ...
        for (const item of inventoryItems) {
            const [existing] = await connection.query('SELECT id FROM inventory_stok WHERE kode_barang = ?', [item.kode]);
            if (existing.length === 0) {
                await connection.query(`INSERT INTO inventory_stok (kode_barang, nama_barang, kategori, tipe_item, jumlah_stok, satuan, harga_standar, stok_committed) 
           VALUES (?, ?, ?, ?, 0, ?, ?, 0)`, [item.kode, item.nama, item.kategori, item.tipe, item.satuan, item.harga_standar]);
        ...
        const [existingBom] = await connection.query('SELECT id_bom FROM manufaktur_bom_header WHERE id_bom = ?', [idBom]);
        if (existingBom.length === 0) {
            await connection.query('INSERT INTO manufaktur_bom_header (id_bom, kode_item_parent, nama_resep) VALUES (?, ?, ?)', [idBom, 'SEP-001', 'Resep Standar MTB Motekar X1']);
            await connection.query(`INSERT INTO manufaktur_bom_detail (id_bom, kode_item_komponen, qty_kebutuhan) VALUES 
         (?, 'KOMP-001', 1),
         (?, 'KOMP-002', 2)`, [idBom, idBom]);
        ...
        const [invCount] = await connection.query('SELECT COUNT(*) as c FROM inventory_stok');
        const [bomCount] = await connection.query('SELECT COUNT(*) as c FROM manufaktur_bom_header');
        const [vendorCount] = await connection.query('SELECT COUNT(*) as c FROM master_vendor');
