...
async function up() {
    ...
    `);
        ...
        if (process.env.NO_SEED !== 'true') {
            ...
            const [fgList] = await connection.query(`
      ...
      `);
            if (fgList.length > 0) {
                ...
                const [insertWO] = await connection.query('INSERT INTO operasi_wo_header (nomor_wo, id_inventory_fg, jumlah_produksi, status) VALUES (?, ?, ?, ?)', [nomor_wo, fg.id, targetQty, 'ON_PROGRESS']);
                const woId = insertWO.insertId;
                // Cari resep BOM
                const [bomHeader] = await connection.query('SELECT id_bom FROM manufaktur_bom_header WHERE kode_item_parent = ?', [fg.kode_barang]);
          ...
          `, [idBom]);
                    for (const detail of bomDetails) {
                        ...
                        await connection.query('INSERT INTO operasi_wo_material_allocation (id_wo_header, id_inventory_material, qty_kebutuhan, status_alokasi) VALUES (?, ?, ?, ?)', [woId, detail.id_material, qtyTotal, 'Reserved']);
                        // Lakukan Soft-Allocation (Tambah stok_committed)
                        await connection.query('UPDATE inventory_stok SET stok_committed = stok_committed + ? WHERE id = ?', [qtyTotal, detail.id_material]);
                    }
