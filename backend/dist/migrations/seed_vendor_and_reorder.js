...
async function seed() {
    ...
    try {
        ...
        for (const pair of pairings) {
            // 1. Insert Vendor
            const [vResult] = await connection.query('INSERT INTO master_vendor (kode_vendor, nama_vendor, status_blacklist) VALUES (?, ?, ?)', [pair.vCode, pair.vName, false]);
            const vendorId = vResult.insertId;
            // 2. Insert RM into inventory_stok
            await connection.query(`INSERT INTO inventory_stok 
        ...
        VALUES (?, ?, 'Komponen', ?, ?, 'RM', 50000, ?, ?, ?, ?)`, [pair.kCode, pair.kName, pair.stok, pair.satuan, vendorId, pair.rop, pair.min, pair.bomRatio]);
        }
        // Re-enable FK checks
