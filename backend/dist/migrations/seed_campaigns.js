...
async function seed() {
    try {
        ...
        const [rows] = await pool.query('SELECT COUNT(*) as count FROM pemasaran_campaigns');
        if (rows[0].count > 0) {
            console.log('Tabel pemasaran_campaigns sudah berisi data. Melewati proses seeding.');
            process.exit();
        }
        for (const c of campaigns) {
            await pool.query('INSERT INTO pemasaran_campaigns (nama_campaign, jenis, budget_alokasi, status) VALUES (?, ?, ?, ?)', [c.nama, c.jenis, c.budget, c.status]);
        }
